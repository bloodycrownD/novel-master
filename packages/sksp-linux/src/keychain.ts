/**
 * Linux Secret Service master key storage and test passthrough.
 *
 * @module keychain
 */
...
const SERVICE = "novel-master-linux";
const USER = "sksp-master-v1";
const MASTER_KEY_BYTES = 32;

/** Test hook: bypass Secret Service with a fixed in-memory master key. @internal */
let testPassthrough = false;
let testMasterKey: Uint8Array | undefined;

/** Enables passthrough mode for non-Linux unit tests. @internal */
export function setLinuxKeychainTestPassthrough(enabled: boolean): void {
...
function parseMasterKey(encoded: string, ref: string): Uint8Array {
  ...
  if (buf.length !== MASTER_KEY_BYTES) {
    throw new SkspError(
      "DECRYPT_FAILED",
      `Invalid master key in Secret Service for ${ref}. Re-run: nm provider edit --apiKey`,
      { ref },
    );
  }
...
function keychainError(
  ...
  const message =
    code === "ENCRYPT_FAILED"
      ? `Secret Service access failed for ${ref}`
      : `Secret Service access failed for ${ref}. Re-run: nm provider edit --apiKey`;
  return new SkspError(code, message, { ref, cause });
}

/**
 * Returns the 32-byte application master key from Secret Service, creating it on first use.
...
export async function getOrCreateMasterKey(
  ...
  if (!isLinux()) {
    throw new SkspError(
      ...
      operation === "encrypt"
        ? "Linux Secret Service (GNOME Keyring / KWallet) is only available on linux"
        : `Linux Secret Service (GNOME Keyring / KWallet) decrypt failed for ${ref} (not on linux). Re-run: nm provider edit --apiKey`,
      { ref },
    );
  }
