 ...
 *
 * 覆盖：
 *  1. bootstrap 后七条内置规则种入（rule_id / sort_order / flags /
 *     captureKind / description，D13 后 4→7）；
 *  2. seed 幂等：INSERT OR IGNORE 不覆盖存量行（禁用+改名后重跑保持原样，
 *     D3「禁用某内置规则后重启保持禁用」）；
 *  3. 用户规则与 seed 隔离（不被覆盖也不被清掉）；
 *  4. 附录 A 常量锁：七条 pattern 均可编译、命中出厂样例文件名且捕获组
...
describe("内置智能排序规则 seed", () => {
  it("bootstrap 后七条内置规则按附录 A 种入（D13 重排 1..7、三条 fixed 档在前、en-chapter flags='i'）", async () => {
    const conn = await openBootstrappedConn();
    try {
      const rows = await conn.query<RuleRow>(
        `SELECT rule_id, name, pattern, flags, capture_kind, description, enabled, sort_order
      ...
      // 捕获档位：三条 fixed 档在前、四条 smart 档顺延（D13）。
      assert.deepEqual(
        rows.map((r) => r.capture_kind),
        ["fixed_min", "fixed_max", "fixed_max", "smart", "smart", "smart", "smart"]
      );
      ...
      for (const row of rows) {
        ...
        assert.equal(row.capture_kind, src.captureKind);
  ...
  it("seed 幂等：禁用+改名后的内置规则不被覆盖（D3）", async () => {
    ...
    try {
      ...
      assert.equal(Number(all[0]?.n ?? 0), 7);
    } finally {
      await conn.close();
    }
  });
  ...
  it("附录 A 常量锁：七条 pattern 可编译、命中出厂样例、捕获档位正确", () => {
    // description 已是描述文字（fix ④），常量锁改用内联样例文件名。
    const SAMPLE_BY_RULE: Record<string, string> = {
      "builtin-zh-prologue": "楔子 起源",
      "builtin-zh-finale": "终章 大结局",
      "builtin-zh-extra": "番外一 日常",
    ...
    const vc = BUILTIN_SMART_SORT_RULE_ROWS.find(
      (r) => r.ruleId === "builtin-zh-volume-chapter"
    )!;
    const vcMatch = "第2卷 第13章".match(new RegExp(vc.pattern));
    assert.ok(vcMatch != null);
    assert.equal(vcMatch[1], "2");
    assert.equal(vcMatch[2], "13");

    // 中文序号章节：捕获组为中文数字原文（数值转换是 Step 4 比较器的职责）。
    const zh = BUILTIN_SMART_SORT_RULE_ROWS.find(
      (r) => r.ruleId === "builtin-zh-chapter"
    )!;
    const zhMatch = "第十二章 风起".match(new RegExp(zh.pattern));
    assert.ok(zhMatch != null);
    assert.equal(zhMatch[1], "十二");

    // 英文章节：flags 'i' 支撑小写命中。
    const en = BUILTIN_SMART_SORT_RULE_ROWS.find(
      (r) => r.ruleId === "builtin-en-chapter"
    )!;
    const enMatch = "chapter 12".match(new RegExp(en.pattern, "i"));
    assert.ok(enMatch != null);
    assert.equal(enMatch[1], "12");

    // 数字序号开头：前导零捕获原文（001、→ '001'，Number() 后为 1）。
    const num = BUILTIN_SMART_SORT_RULE_ROWS.find(
      (r) => r.ruleId === "builtin-numeric"
    )!;
    const numMatch = "001、开端".match(new RegExp(num.pattern));
    assert.ok(numMatch != null);
    assert.equal(numMatch[1], "001");

    // 前置必要性（附录 A 说明 1）：zh-chapter 单独命中「第2卷 第13章」只提卷号。
    const preemption = "第2卷 第13章".match(new RegExp(zh.pattern));
    assert.ok(preemption != null);
    assert.equal(preemption[1], "2");

    // D13：三条固定档规则的档位正确（pattern 含 alternation 分组也无妨：
    // fixed 档忽略捕获组，固定元组优先——T-SS 已锁提取语义）。
    const kindById = new Map(
      BUILTIN_SMART_SORT_RULE_ROWS.map((r) => [r.ruleId, r.captureKind])
    );
    assert.equal(kindById.get("builtin-zh-prologue"), "fixed_min");
    assert.equal(kindById.get("builtin-zh-finale"), "fixed_max");
    assert.equal(kindById.get("builtin-zh-extra"), "fixed_max");
    // 四条数字规则显式 smart（行为零变化）。
    for (const id of [
      "builtin-zh-volume-chapter",
      "builtin-zh-chapter",
      "builtin-en-chapter",
      ...
      assert.equal(kindById.get(id), "smart");
    }
  });
});
