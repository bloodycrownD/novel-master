 ...
 */
async function seedLegacyUnconstrainedSchema(
  conn: TdbcConnection
): Promise<void> {
...
describe("T-CT1: 脏值预扫描 + 清洗（table-constraints-v1）", () => {
  it("非法 role / hidden / seq、NULL PK、负 ref_count、非法 algo / json / flags 等被清洗或丢弃", async () => {
    ...
    assert.equal(
      rev[0]!.status,
      "deleted",
      "active + NULL content_hash 应清洗成 deleted"
    );
    ...
    assert.equal(
      k2[0]!.algo,
      "dpapi-v1",
      "非 dpapi + NULL iv 应清洗 algo 成 'dpapi-v1'"
    );
