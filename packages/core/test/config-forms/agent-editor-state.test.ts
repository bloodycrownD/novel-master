...
test("T-SA1: definitionToForm 将 subagentCallable===true 映射为表单 true", () => {
  const form = definitionToForm({
    name: "x",
    prompts: { persist: [], dynamic: [] },
    subagentCallable: true,
  });
  assert.equal(form.subagentCallable, true);
});

test("T-SA2: definitionToForm 对缺省/非 true 值返回 false", () => {
  const omitForm = definitionToForm({
    name: "x",
    prompts: { persist: [], dynamic: [] },
  });
  assert.equal(omitForm.subagentCallable, false);
  const falsyForm = definitionToForm({
    name: "x",
    prompts: { persist: [], dynamic: [] },
    subagentCallable: undefined,
  });
  assert.equal(falsyForm.subagentCallable, false);
});

test("T-SA3: buildAgentDefinitionFromForm 在 subagentCallable===true 时透传 true", () => {
  const result = buildAgentDefinitionFromForm({
    name: "writer",
    maxSteps: "20",
    modelEnabled: false,
  ...
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.definition.subagentCallable, true);
  }
});

test("T-SA4: buildAgentDefinitionFromForm 在 subagentCallable 关时省略字段", () => {
  const result = buildAgentDefinitionFromForm({
    name: "writer",
    maxSteps: "20",
    modelEnabled: false,
  ...
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.definition.subagentCallable, undefined);
  }
});

test("T-SA5: formSnapshotJson 将 subagentCallable 纳入 dirty 比对", () => {
  const base = createDefaultAgentEditorPrompts();
  const json = formSnapshotJson({
    name: "a",
    maxSteps: "20",
  ...
  assert.equal(parsed.subagentCallable, true);
  // 缺省时序列化为 false
  const jsonOff = formSnapshotJson({
    name: "a",
    maxSteps: "20",
    modelEnabled: false,
    providerId: "",
  ...
  assert.equal(JSON.parse(jsonOff).subagentCallable, false);
});
