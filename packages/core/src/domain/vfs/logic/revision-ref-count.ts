...
export async function repairRefCounts(
  ...
  // 丢给 repairRefCountFloorBatch，它内部按 500 分块批量 SELECT + conn.batch UPDATE，
  ...
  const rowsAdjusted = await revisionRepo.repairRefCountFloorBatch(items);
 ...
 * 具体偏差交给幂等的 repair 自己处理（`repairRefCountFloorBatch` 只增不减，重复跑安全）。
 *
 ...
 * `repairRefCountFloorBatch` 只更新 `ref_count` 列、不改 `content_hash`，触发器不会 fire，
