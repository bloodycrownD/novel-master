...
- CI 内会对 desktop / mobile 再执行 `npm version <tag名无v> --no-git-tag-version`；**仓库里仍应先改好 `package.json` 再发 tag**，这样 dev 构建与关于页在发版前即正确，且 tag 指向的 commit 自洽。
