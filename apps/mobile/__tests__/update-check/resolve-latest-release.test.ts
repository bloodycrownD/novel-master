...
describe('resolveLatestRelease', () => {
  ...
  it('rejects evil release domains', async () => {
    const mockFetch = jest.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
        tag_name: 'v2.1.0',
        html_url:
          'https://evil.example/bloodycrownD/novel-master/releases/tag/v2.1.0',
        body: '',
      }),
    });

    await expect(
      resolveLatestRelease(mockFetch as typeof fetch),
    ).rejects.toThrow('发行版链接域名校验失败');
  });
});
