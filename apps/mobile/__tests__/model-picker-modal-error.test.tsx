/**
 * ModelPickerModal 错误处理（cr-fix-spec b2/B-1、b2/B-2）：
...
jest.mock('@novel-master/core/provider', () => {
  // 保留真实导出：format-error.ts 也从该入口拿 ProviderError，
  // 全量替换会把错误类抹成 undefined 导致 instanceof 崩。
  const actual = jest.requireActual('@novel-master/core/provider');
  return {
    ...actual,
    formatSavedModelDisplayName: (provider: string, model: string) =>
      `${provider} / ${model}`,
  };
});
...
const mockGetCurrentModelId = jest.fn();
const mockSetCurrentModelId = jest.fn();

const mockRuntime = {
  state: {
    getCurrentModelId: mockGetCurrentModelId,
    setCurrentModelId: mockSetCurrentModelId,
  },
...
async function renderPicker(
  props: Partial<React.ComponentProps<typeof ModelPickerModal>> = {},
) {
...
const SAVED = [
  {id: 'm1', providerId: 'p1', modelName: 'gpt-4o', vendorModelId: 'gpt-4o'},
];

describe('ModelPickerModal 错误处理', () => {
  beforeEach(() => {
    mockList.mockReset().mockResolvedValue(PROVIDERS);
    mockSavedList.mockReset().mockResolvedValue(SAVED);
    mockGetCurrentModelId.mockReset().mockResolvedValue('m1');
    mockSetCurrentModelId.mockReset().mockResolvedValue(undefined);
  ...
  it('B-1: 加载失败渲染错误文案与重试，空态文案不出现', async () => {
    ...
    expect(text).not.toContain('暂无已保存模型');
  });
  ...
  it('B-1: 点重试成功后恢复列表', async () => {
    ...
    expect(json(renderer)).toContain('OpenAI / gpt-4o');
    expect(json(renderer)).not.toContain('服务商读取失败');
  });

  it('B-2: 选中写入失败 toast「设置失败」且不回调不关闭', async () => {
    mockSetCurrentModelId.mockRejectedValueOnce(new Error('写入炸了'));
    const {renderer, onClose, onSelected} = await renderPicker();

    await act(async () => {
      findClickableByText(renderer.root, 'OpenAI / gpt-4o')!.props.onPress();
    });
  ...
  it('B-2: 写入成功才 onSelected + onClose', async () => {
    ...
    expect(mockSetCurrentModelId).toHaveBeenCalledWith('m1');
    expect(onSelected).toHaveBeenCalledWith('m1');
