 ...
 *
 * @deprecated Transcript path uses {@link ChatTranscriptWebView}; retained only as the
 * rollback path when `chatTranscriptEngine` KKV is set to `legacy-rn`
 * (see storage/chat-transcript-engine.ts). Do not re-enable as primary UI.
 *
...
const ChatMessageBody = React.memo(function ChatMessageBody({
  ...
  return (
    <RichContentBody content={body} tokens={tokens} fallbackTextColor={plainColor} />
  );
});
