 ...
 */
export function thinkingBodyInner(text, thinkingHtml) {
...
export function renderThinkingSection(text, key, expanded, thinkingHtml, showDividerBelow) {
    var trimmed = String(text || '').trim();
    if (!trimmed) return '';
...
export function renderAttachGroupSection(attachments, key, expanded, showDividerAbove) {
...
export function renderToolOnlyBubble(tools, toolGroupKey, toolGroupExpanded, options) {
...
export function renderUserVfsTurnRow(row) {
...
export function renderUserBubbleContent(text) {
    // VFS 操作卡只走结构化 row.kind === 'user_vfs_turn'；正文不再兜底解析 <action>
    return escapeHtml(text);
  }

export function renderMessageRow(row) {
...
export function renderLoadOlder() {
