...
/** Session VFS subcommands except `write`（write 显式带 --version / --no-version-check）。
 */
...
async function runSessionVfs(deps: SessionDeps, args: readonly string[]): Promise<void> {
  ...
  if (group === "import-character-card") {
    const idx = args.indexOf(group);
  ...
  if (group === "write") {
    // 版本校验改为显式 opt-in：--version 乐观锁 / --no-version-check 依旧可用，
    // 缺省 last-write-wins（原先由 session-fs.versionCheck 偏好决定，已移除）。
    await runWrite(vfs, subArgs, { defaultNoVersionCheck: true });
    return;
  }

  if (group == null || !(group in SESSION_VFS_COMMANDS)) {
    throw new Error(
      "Usage: nm session vfs <list|read|write|export-zip|import-zip|import-character-card|...>",
    );
  }
