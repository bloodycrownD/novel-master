...
describe("agent registry e2e", () => {
  ...
  it("E4 / T-C3: tools + subagentCallable 导入导出闭环", async () => {
    ...
    try {
      ...
      // show 导入后的 agent，验证 tools + subagentCallable 保留
      const shown = runNm(["agent", "show", "researcher", "--db", dbPath]);
      assert.equal(shown.status, 0, shown.stderr);
      assert.match(shown.stdout, /"subagentCallable"\s*:\s*true/);
      ...
      assert.match(shown2.stdout, /"subagentCallable"\s*:\s*true/);
