...
- **「思考提示词」开关**（双端）：聊天设置中可控制模型的思考内容是否进入后续请求的上下文，默认开启；开启后最近一次的思考进入提示词，更早的自动刷新不进入。anthropic / gemini 以原生思考块回传，OpenAI 兼容协议（GLM、DeepSeek 等）以 `reasoning_content` 字段回传，三家协议行为一致

### 变更

- **思考内容进入上下文的默认行为调整**：模型的思考内容不再随每次请求完整回传，仅保留最近一次思考（含进行中工具调用所需的部分），上一轮思考对新一轮可见、更早的自动刷新——已有长会话升级后首次发起请求时，服务商的前缀缓存可能失效一次，带来一次性的成本与延迟波动
