...
- **「思考提示词」开关**（双端）：聊天设置中可控制模型的思考内容是否进入后续请求的上下文，默认关闭（与旧版行为一致、避免兼容风险）。开启后历史思考全部保留：anthropic / gemini 以原生思考块回传，OpenAI 兼容协议（GLM、DeepSeek 等）以 `reasoning_content` 字段回传；关闭则不进入
