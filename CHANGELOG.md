...
- **「思考提示词」开关**（双端）：聊天设置中可控制模型的思考内容是否进入后续请求的上下文，默认开启；开启后仅最新一轮思考进入提示词，历史思考不进入

### 变更

- **思考内容进入上下文的默认行为调整**：模型的思考内容不再随每次请求完整回传，仅保留最新一轮（含进行中工具调用所需的部分）——已有长会话升级后首次发起请求时，服务商的前缀缓存可能失效一次，带来一次性的成本与延迟波动
