...
- **工作区内容去重存储**：会话文件正文按内容哈希共享存储并压缩，复制 / 分叉 / 回填不再整份拷贝正文，升级库时分批迁移，降低体积与大库闪退风险
- **Composer 操作日志**：手改以按次操作日志为真源（投影 chip、发送门闩、停写旧 pending）；跨目录移动与同目录改名分流展示
- **Mobile 工作区长按多选移动**：批量栏支持多选后移动到目标目录
- **导入角色卡**：Desktop / Mobile / CLI 可将酒馆 PNG 或 JSON 角色卡覆盖导入当前工作区文件夹，生成角色描述、开场（三位编号如 `开场001`）与世界书 Markdown（关键字写入 Front Matter）；语义对齐目录级「导入 ZIP」

### 变更

- **首次打开可能较慢**：本次更新后第一次启动应用会有较长时间加载，属正常情况（正在压缩历史数据）；第二次起即可正常进入
- **消息回滚更快**：同版本 / 同内容跳过无用写盘；引用计数打扫 revision；blob 回收挪出回滚热路径（真机多数同文回滚可压到约亚秒级）
- **Undo Send / Rewind 后手改 chip**：回滚成功后清空未发送操作日志（不再从消息映回手改 chip）；正文与工作区批注仍可按既有规则恢复
- **手改协议补 `move`**：同目录 rename、跨目录 move，双端 Composer / Bridge 对齐

### 修复

- **失败补偿后的垃圾 revision**：递归删除空目录等补偿路径会正确递减引用，便于后续回收不可达历史与孤儿正文
- **会话 / 模板清理**：删会话与会话模板拉取后走统一 revision 打扫与延期 blob 回收，降低误删与残留
