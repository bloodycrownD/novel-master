...
- **AI 联网搜索**（双端）：AI 对话中可调用搜索工具查阅网络资料并附带来源；支持博查、Tavily、Brave 与自托管 SearXNG 实例，搜索引擎在「搜索配置」中手动配置（桌面「设置 → AI → 搜索配置」，移动端「我的 → 配置 → 搜索配置」），Key 经系统安全存储加密保存；内置 DuckDuckGo 作为免密钥兜底——不配置任何引擎也能开箱搜索；多个引擎按配置顺序串行尝试——前面的引擎请求失败时自动降级到下一个
