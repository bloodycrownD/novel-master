...
- **聊天消息里的文件链接可直接打开**（双端）：助手消息中的 markdown 链接（相对/绝对路径、中文路径均支持）点击后自动在应用内打开对应文件——先在聊天工作区找、再在项目工作区找，找到即在预览/编辑器打开，找不到则提示该路径不存在（超长路径中间省略显示，如 /测试.../报告.md 不存在）；http(s) 网址维持系统浏览器打开，子会话页面同样支持（共享父会话工作区）
