...
- **聊天工作区「推送到项目工作区」**（双端）：会话里整理好的设定、大纲、素材可一键回写为项目模板，与「从上级同步」对称——推送后该项目新建的会话与其它会话的「从上级同步」拿到的都是推送后的母本；推送为整树覆盖（含目录规则，母本中多余文件一并删除），操作前有确认弹窗讲清影响面。入口：移动端聊天工作区工具栏（与同步按钮并排）/ 桌面端聊天工作区「更多」菜单
- **统计图表更易读**（移动端）：图表页柱状图新增纵坐标刻度（0、最大值一半、最大值三档，横向滚动时刻度值始终可见），不用再靠猜柱高读量级；汇总页饼图图例常驻显示各切片占比百分比，占比 30% 以上的大切片同时在扇区内直接标注百分比，无需逐个点选

### 变更

- **移除请求中非标准协议字段 tool_stream 的自动注入**（双端）：此前对话请求智谱 GLM 系模型时会自动附加该字段，部分第三方 openai 协议网关严格校验会拒收并报「未知请求字段: tool_stream」；智谱官方直连如需该参数，可在服务商「自定义参数」中配置 {"tool_stream": true}
