...
- **AI 联网搜索**（双端）：AI 对话中可调用搜索工具查阅网络资料并附带来源；支持博查、Tavily、Brave 与自托管 SearXNG 实例，搜索引擎 API Key 在设置中手动配置（桌面「设置 → AI → AI 搜索」，移动端「我的 → 配置 → AI 搜索」），Key 经系统安全存储加密保存；未配置时 AI 会收到配置指引提示
