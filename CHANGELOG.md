...
- **Android 输入框抖动**（移动端）：修复聊天输入框引用文件/技能（着色 tag）且文本较多时，连续输入/删除出现内容上下抖动的问题
...
- **OpenAI 兼容协议模型历史思考进入上下文**：此前 OpenAI 兼容协议（GLM、DeepSeek 等）出站时丢弃思考内容、模型看不到自己的历史思考；现在开启「思考提示词」后以 `reasoning_content` 字段随消息回传（对齐业界通用做法）。关闭开关则思考内容不进入上下文——已有长会话切换开关后首次发起请求时，服务商的前缀缓存可能失效一次，带来一次性的成本与延迟波动
