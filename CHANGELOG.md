...
- **AI 修改已有技能不再报版本冲突**（双端）：对话中让 AI 编辑已存在的技能时，此前会被文件版本校验拒绝且 AI 无法提供所需版本，现在直接覆盖保存；同时精简 AI 写文件工具的参数，移除无效的版本校验字段
...
- **AI 修改智能体改为按需提交**（双端）：对话中让 AI 调整智能体配置时，此前需要先读取完整定义再整体回写，改错或漏抄字段会静默丢失配置；现在只提交要修改的字段（如只改描述或只换一组提示词），未提供的字段保留现值，提交「null」可清除字段恢复默认
- **文件写入版本校验整体移除**（双端）：此前编辑器保存需先读版本再带回校验，与其他写入路径的规则不一致；现在全部写入统一为后写覆盖，版本号仅作内部记录不再比对，双端编辑器与命令行行为一致；「文件版本校验」设置项同步移除，命令行写文件（含会话内写文件）的 `--version` 与 `--no-version-check` 旗标一并移除；命令行从空管道读取输入时报错提示，不再静默清空已存在文件
