...
- **文件写入版本校验整体移除**（双端）：此前编辑器保存需先读版本再带回校验，与其他写入路径的规则不一致；现在全部写入统一为后写覆盖，版本号仅作内部记录不再比对，双端编辑器与命令行行为一致

### 变更

- **对话中保存类工具的结果展示统一**（双端）：AI 保存技能、创建或修改智能体后，工具卡片不再展开一段配置 JSON，与文件写入一致显示「ok」；AI 加载技能时改为带行号的可读正文（附附属文件清单），不再转义展开
- **移除「文件版本校验」设置项与命令行版本旗标**（双端）：该开关对应的行为已随写入链路简化下线，设置界面同步移除；命令行会话写文件的 `--version` 与 `--no-version-check` 旗标一并移除
