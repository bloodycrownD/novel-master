...
- **hidden 消息**：对 LLM 提示词渲染隐藏的标记——落库保留但不进提示词。只由两种操作产生：压缩和置位。→ `packages/core/src/domain/chat/model/message.ts`（`hidden` 字段）、过滤逻辑：`packages/core/src/domain/agent/session/`
...
- **压缩（compaction）**：系统按 token 条件自动从尾部隐藏最旧一段消息。副作用同置位。→ `packages/core/src/service/compaction-conditions/run-compaction.ts`
...
- **KKV（session KKV）**：按 sessionId 路由的键值存储，只有三个域：`rule_snapshot`（工作区规则快照）、`file_cache`（展示正文缓存，性能优化）、`user_vfs_pending`（用户操作 FIFO 队列）。`file_cache` 不能当「前文是否引用过」的判断依据。→ `packages/core/src/domain/session-kkv/`（域定义：`model/session-kkv-domains.ts`）
