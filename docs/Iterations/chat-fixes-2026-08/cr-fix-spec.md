...
must-fix 共 7 条，全部 P2（无 P0/P1）。维度图例：C=core、A=android/mobile、G=general（跨端/测试基建）。

## Must-fix（按 P0 → P1 → P2）

本轮无 P0、P1；以下为 7 条 P2。
...
- **文件**：`packages/core/src/service/agent/logic/run-agent-turn.ts:118-120`
...
- **文件**：`packages/core/test/depth/resolve-hide-message-range.test.ts`（用例补入该文件）
...
- **文件**：`apps/mobile/src/screens/tabs/chat-tab/useChatTabMessages.ts`（unhide 分支，约 L694-712）
...
- **文件**：`apps/mobile/__tests__/chat-composer.integration.test.tsx`（用例补入该文件）
...
### review-full/C-2b [P2] [C] `synthesize-user-vfs-flush-actions` 拆除后成为孤儿模块

- **文件**：
  - `packages/core/src/domain/chat/logic/synthesize-user-vfs-flush-actions.ts`
  - `packages/core/test/chat/synthesize-user-vfs-flush-actions.test.ts`
- **问题**：该模块（含 `UserOpsActionSummary` / `synthesizeUserVfsFlushActions` / `collectUserOpsActionSummaries`）的生产消费方已全部删除（`user-vfs-turn.port.ts` 类型引用已改写、`project-composer-status-attachments.ts` 与 `build-user-ops-attachment.ts` 整文件删除），现仅剩自身单测在引用——拆除收尾漏网的死代码模块。
- **改法**：src 文件与配套测试文件一并删除（dist 产物随构建刷新，不用管）。
- **验收/测试**：`grep -rn "synthesize-user-vfs-flush-actions\|UserOpsActionSummary" packages/ apps/`（排除 dist）零残留；core 全量测试仍绿。
- **来源**：review-full r2（CLI 盲区补查时认定）
...
2. **`AgentTurnRuntimePort.userVfsTurn` 成员去留**：core 内已零消费，三端装配方仍传（mobile/desktop runtime 及 CLI parity 测试 `makeRuntime` 均在传）；是否跨端删除（含 CLI 装配方）待拍板。
