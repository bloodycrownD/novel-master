...
- T-L5 — blocking: yes — Step 3：RN 路由函数测试（mock vfs list/Linking）：session 命中→FileEditor(session)；仅 project 命中→FileEditor(project)；双未命中→无动作；http(s)→Linking；子会话用 parentSessionId（照 `file-editor-screen` / chat-tab 集成测试模式 + core-shim 补导出）。
