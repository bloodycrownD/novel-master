...
- 模型筛选：默认「全部模型」，可选具体模型；模型选项列表来自库内已记录的模型标识（含「未记录」）
...
- 范围内：总 token、输入 token、输出 token、模型调用次数、缓存命中率
- 今日：总 token、调用次数（不受时间筛选影响，始终为今日）
...
- 汇总列表：当前时间筛选内每个模型的用量、调用次数、命中率、总量占比
