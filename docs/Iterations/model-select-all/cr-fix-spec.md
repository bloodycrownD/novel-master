...
## Open questions / 待拍板
1. desktop 弹窗批量保存部分失败语义是否向 mobile 看齐（现状：失败 toast 后 return，已成功行不标记、重试会重复 save）？或接受维持现状（预存行为，非本 diff 引入）。
2. mobile 保存中所有已勾选未保存行同时转圈（含未轮到的），desktop 无此指示——是否接受。
3. `provider-detail-tabs.test.ts` 源码正则断言风格（脆但符合该文件既有风格）——是否接受。
