...
**迭代收尾状态（2026-08-28）**：R2/R3 全部完成并验收；R1 抖动已修（去 padding），tag 多行闪烁经三轮实验（wrapper 复用/库短路/顶限测试）定性为 TextView 层面不可避免，用户拍板接受现状、择期修复，候选方案（编辑时降级纯文本/overlay）与死路结论已归档至 RULE.md 与记忆。
