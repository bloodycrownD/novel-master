...
| 25 | `patches/react-native-controlled-mentions+3.1.0.patch` + 根 `package.json` | 【已实施后回滚（真机 tag 消失复现）】patch-package 方案：库内短路冗余 children 重推（原生上报文本+mention 签名双判据）。真机日志证实 skip 生效但 tag 随即丢失——结论：RN Android 的 TextInput 里 children 重推是 span 存续的载体，短路不可行；库架构下「打字→重推→重建→闪烁」绑死。补丁、postinstall、T-C3/T-C4 均已回滚移除。后续方向：①去胶囊背景实验（仅色字/加粗，闪烁可能不可感知）；②透明输入+高亮层 overlay 重写（根治，工程量大） | 用户真机反馈 |
...
- Step 24 — phase-composer-stable — blocking: yes — qa: manual_user：patch-package 短路库冗余 children 重推（变更点 #25，T-C3/T-C4 auto 锁定 JS 层行为；真机验收 tag 不闪烁且不消失）。【已回滚：真机 skip 生效但 tag 消失，短路不可行，见 #25 结论】
...
**迭代收尾状态（2026-08-28）**：R2/R3 全部完成并验收；R1 抖动修复（去 padding）经后续真机复验无感知差异，已回退保持原样式（main d628694）；tag 多行闪烁经三轮实验（wrapper 复用/库短路/顶限测试）定性为 TextView 层面不可避免，用户拍板接受现状、择期修复，候选方案（编辑时降级纯文本/overlay）与死路结论已归档至 RULE.md 与记忆。
