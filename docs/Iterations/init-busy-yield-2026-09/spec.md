...
3. **backfill 低成本判定**（core）：发送链路上两段式判定「无空窗」短路——第一段 O(1) 计数比对（`countBySession` == 游标直接短路；count < 游标即消息数减少、删除可疑态，并入矛盾检测回退全量），第二段对游标边界之后的新增段做 O(新增段) 有界覆盖比对（覆盖口径与空窗语义等价：段内每条消息含 assistant 均有 checkpoint；组合闲置原语 `hasAnyCheckpointForSession` / `countBySession`）；判定不确定时**保守回退全量扫描**（回滚保证宁误报勿漏报）；`session_kkv` 挂扫描游标消除常态扫描，删除路径（truncate-tail、单条删除 `delete(id)`、`truncateAfter` 含清空分支）一律清游标（seq 复用坑）。
4. **快照链路分片**：
...
- T-B5 — blocking: yes — 两段式第二层短路（连续对话）：会话已确认无空窗（游标=N）后追加一轮「源头已建点」的新消息（发送链 user capture + mutating 工具轮 assistant capture，共 2 条，即连续对话上一轮的产出形态），第二次 run 开头 backfill：count=N+2 > 游标 → 新增段有界覆盖比对通过（段内 2 条均有 checkpoint，与空窗语义等价）→ 短路 no-op、不回退全量扫描查询、游标前移至 N+2。反向场景（守住「真实空窗必须补」底线）：游标=N 后追加一轮纯文本（user 有 capture、尾部 assistant 无源头建点，共 2 条），第二次 run：count=N+2 > 游标 → 圈段内 assistant 无 checkpoint → 覆盖比对失败 → 回退全量补建 + 游标补写至 N+2（Step 4）。
- T-B6 — blocking: yes — 删除路径回退全量：其一，中间删除后新增无点消息必须回退全量并补建——游标=N、经 `delete(id)` 中间删除（如 tool-turn-actions.ts:43-46 中间删工具轮对）后游标已清，随后新增无 checkpoint 消息，下次 run 游标缺失→回退全量补建+游标补写；若删除路径漏清游标致 count < N，亦被第一段删除可疑态判定兜回全量。其二，清空会话（`truncateAfter` anchor=null 清空分支）重聊场景：游标已清、首轮回退全量（Step 4）。
