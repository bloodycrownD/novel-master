...
- 读路径：`ChatStreamMetricsBarLive`（或等价消费方）的数据源为**终态单元快照优先、settled 投影兜底**——有单元投影时直接用（活跃 starting/running 显示实时计时；settled 含 interrupted 显示冻结现场，interrupted 历时以 settledAtMs-startedAtMs 近似，水合被杀于 starting 阶段的全零快照跳过显示），无单元时退 manager 的 settled 投影（「上次生成」跨重启）。stream-metrics-store 退役后不再依赖 store 单例。settled 投影为 **manager 级常驻 map**（sessionId → metrics 快照），独立于单元生命周期：不随单元宽限销毁或 LRU 淘汰清除（否则单元淘汰后「上次生成」断源），仅被同会话新 run 收尾时的 settled 写入覆盖，或随会话删除清理。（实现演进注：初稿为「仅 settled 投影」单源，因 interrupted 行不写 settle、投影断源无法满足 PRD「指标恢复」，落地为快照优先双层源——见 chat-stream-metrics-bar-live 组件测试。）
