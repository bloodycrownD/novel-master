...
## 需求变更追记（2026-09-09，Step 5 真机验证反馈）

1. **保活通知内容化**：常驻「正在生成」通知携带当前项目名 + 会话名（标题 `正在生成 · 会话名`，正文 `项目名 · 会话名`）；多会话并行时显示最近发起的会话并附「共 N 个会话生成中」。通知模块按 sessionId 维护标签集，单会话收尾摘标签、通知内容原位刷新，最后一个收尾才停服务（起停串行化链与既有竞态语义不变）。
2. **后台保活独立开关（默认关）**：新增偏好 `agentKeepAlive`（appUi 通道，模块 `nm-mobile-ui`），默认关闭——历史行为完全兼容（无常驻通知、无前台服务，退后台请求按升级前口径可能被中断）。开启后才在受理时起前台保活服务。与「生成结束通知」开关（默认开）相互独立：完成通知不依赖保活开启。
...
### 追记补充（同日第四轮，用户拍板：回滚聚合）

7. **回滚 per-session 通知，维持聚合单条**：per-session + FGS 载体模型在真机上仍现「点按后通知消失」（华为 EMUI 对 FGS 通知点按的处置与 AOSP 预期不符，加 pressAction 后依旧）。用户拍板回到聚合单条模型（追记 1 的形态：单条常驻通知、内容随最近发起的会话、多会话附总数、开关默认关）。代码恢复至追记 1 落地版（602cb909 的通知模块/测试/mock）；开关文案保留精简版「常驻通知」。点按直达会话仅保留在完成通知上（带 pressAction，链路已验证）；常驻通知点按无自定义行为（打开应用）。
