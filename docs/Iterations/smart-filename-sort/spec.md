...
| D13 | 捕获数字三档（captureKind） | 规则加 `capture_kind` 枚举 `smart \| fixed_min \| fixed_max`（DDL TEXT NOT NULL DEFAULT 'smart' + CHECK，缺省 smart 行为零变化）。**smart** = 现有捕获组→数字转换管道不变（chineseNumToInt 兼容 \d）；**fixed_min** 命中即哨兵元组 `(-Infinity,)` 排一切序号前（序章/楔子类）；**fixed_max** 命中即哨兵元组 `(+Infinity,)` 沉底（终章/番外类）。固定档**忽略捕获组**（有无捕获组均可、validate 不再强制 ≥1 捕获组、捕获组共存合法——固定元组优先）；哨兵 ±∞ 仅存于内存比较，不落库不进 YAML/IPC 序列化（存储层只有枚举字符串，比较器对 ±∞ 数值比较天然成立）；固定档同值按原文件名自然序 tiebreak 决胜（「番」U+756A <「终」U+7EC8 → 同 fixed_max 下番外排在终章前，已验证；定稿不加不同权重，测试锁行为）。内置规则 4→7 条：三条 fixed 档排最前（prologue=1 / finale=2 / extra=3），四条数字规则显式 smart 顺延 4..7（见附录 A）。存量库走 pending migration `add-smart-sort-capture-kind-v1`（PRAGMA table_info 探测无列才 ALTER ADD COLUMN，存量行自动默认 smart 无需回填，SCHEMA_BOOT_VERSION 不 bump）；bundle v2 加 optional `captureKind` 字段（向后兼容，不 bump schemaVersion）；matchSmartSortPattern 每匹配增 `tuple` 展示字符串（smart 档 "(12,)" 风格 / 固定档 "(固定最小,)"/"(固定最大,)" / 无捕获组或转换失败 null，GUI 显「无序号」） | 两轮用户讨论拍板；node 验证中文 code unit 序 |
...
【重构注 2026-09-12】本表的 example 列及附录 A 各 example 行为历史形态记录：字段已更名 description、值为描述文字（如 builtin-zh-volume-chapter「匹配 第X卷…第Y章 复合结构，序号取 [卷,章]」）；存量库经 rename-smart-sort-rule-example-v1 迁移改名并刷内置行出厂描述，bundle schemaVersion 1→2（decode 兼容旧 example 字段）。下表/附录中的样例文件名仅作 pattern 行为验证用途保留。
...
- Step 9 — phase-cli — blocking: yes — qa: auto：`main.ts` 注册 `"sort-rule"`；`sort-rule/commands.ts`：`list`（TSV：order⇥id⇥enabled⇥flags⇥name⇥pattern）、`create --name --pattern [--flags] [--example]`、`update --id [--name] [--pattern] [--flags] [--example]`、`remove --id`（对 `builtin-` 前缀报错，D3）、`enable|disable --id`、`move --id --to top|bottom|up|down|<N>`、`import --file <path>` / `export --file <path>`（走 core `parseText/stringifyText` + `smart-sort-rule-io.ts` decode/encode，参照 `agent/import-export.ts`）、`test <name...>`（positional 多文件名，输出逐行 `文件名⇥命中规则id⇥序号元组` + 空行 + 排序后列表）；`run-workplace.ts:148` `parseSortField` 加 `'smart'`。【D13 注 2026-09-13】`create`/`update` 增 `--capture-kind <smart|fixed_min|fixed_max>`（缺省 smart，非法值报 invalid）；`list` TSV 增列插在 enabled 后：order⇥id⇥enabled⇥capture⇥flags⇥name⇥pattern；`test` 明细对固定档显示「固定最小/固定最大」文案（不直出 ±Infinity）。
- Step 10 — phase-desktop-ipc — blocking: yes — qa: auto：`ipc-types.ts`：`IPC_CHANNELS` 加 `nm:sort-rule/*`（list/create/update/delete/setEnabled/move/reorder/setEnabledBatch/deleteBatch/importRules/exportRules/resetDefaults/preview）+ DTO/Request（含 `flags` 字段）+ `WorkplaceSetDirRuleRequest.sortField` union 加 `'smart'`；`handlers/smart-sort-rule.ts`（IpcResult 范式）+ `handler-registry.ts` bind + `invoke-registry.ts` 封装 + desktop runtime 装配两步。【重构注 2026-09-12】preview 通道改语义为正则匹配测试：通道更名 `nm:sort-rule/match`（SMART_SORT_RULE_MATCH），Request 改 `{pattern, flags, text}`，handler 直调 core 新纯函数 `matchSmartSortPattern`（不再经 runtime.previewSort），非法正则作为 `ok:false` 结果内联返回而非 IPC 错误；CLI `sort-rule test`（previewSort）保留不变。
- Step 11 — phase-desktop-ui — blocking: yes — qa: auto：`settings-nav.ts` ViewId `smartSortRules`（「高级」组）+ navState（editingSmartSortRuleId）+ 高亮归并；`SettingsViews.tsx` 新视图：列表（启用 Switch（`ui/Switch.tsx`）+ name + example 小字 + ContextMenu（编辑/上移/下移/置顶/置底/删除，**`builtin-` 行隐藏删除项**（D3：内置仅可禁用），删除走 ConfirmModal 确认——不照抄 RegexRulesView 裸删行为）+ ManageHeader 批量（先扩 D12 actions）+ HTML5 拖拽 + 导入/导出/恢复默认按钮（yaml 走 main 进程 `yaml-shared.ts` 通道，新增 `smart-sort-rule-yaml.service.ts`，schema/encode/decode 引 core 单源 `smart-sort-rule-io.ts`）；编辑视图：名称/正则/**正则输入支持 /pattern/flags 字面量格式（core parsePatternInput 单源），无独立 flags UI；全量 flags 仍可经字面量/CLI/YAML**/示例表单 + 启用开关 + 测试预览（多行输入 → IPC preview → monospace `<pre>`，参照正则编辑器 footer 按钮模式）。【重构注 2026-09-12】真机验收后重构：① 编辑器测试预览由排序测试改为正则匹配测试（textarea 输入文本 + 「测试」按钮手动触发，删实时联动与列表试跑，结果区逐匹配显示文本+捕获组）；② example 字段更名 description（表单标签「描述」、列表小字同步）；③ formatPatternInput 回显统一 /pattern/flags（flags 空为 /pattern/）。【重构注 2026-09-12 二次】测试结果区改高亮渲染：原文内匹配段主题色（--primary）变色 + 底部「共 N 处匹配」计数（替代"文本"[组]清单；core matches 增 index 偏移，按偏移切分不因重复文本错位）；描述字段改 textarea rows=3 多行。【D13 注 2026-09-13】编辑器增「捕获数字」原生 select（三选项：智能数字/固定最大/固定最小，档位描述 hint 随动）；测试预览每处匹配下显示提取元组小字（tuple 文案，null 显「无序号」，固定档显哨兵文案）；match Request 增 captureKind 透传。
- Step 12 — phase-desktop-ui — blocking: yes — qa: auto：`DirectoryRuleModal.tsx`：`SORT_FIELDS` 加「智能排序」；分组文案「排序字段」→「排序方式」（`:167`）。
- Step 13 — phase-mobile-ui — blocking: yes — qa: auto：runtime 装配两步；导航注册五处（`types.ts`/`RootNavigator.tsx`/`header-config.ts`/`ProfileTabScreen.tsx` CONFIG_MENU）；`SmartSortRulesScreen`（列表 + Switch + ⋮ BottomSheetMenu（编辑/上移/下移/置顶/置底/删除，**`builtin-` 行隐藏删除项**（D3））+ `useBatchSelection`/`useBatchDeleteConfirm`/`ManageHeader actions` 批量启停删 + 导入导出（`yaml-shared.ts`+`document-io.ts`，导入 Alert 确认）+ 恢复默认）；`SmartSortRuleEditorScreen`（FormSectionCard 表单（名称/**正则输入支持 /pattern/flags 字面量格式（core parsePatternInput 单源），无独立 flags UI；mobile 正则字段全屏编辑（照 agent 配置先例）；全量 flags 仍可经字面量/CLI/YAML**/示例）+ 测试预览区，照抄 RegexRuleEditorScreen 的 useCallback+useEffect 联动模式，预览调 service.previewSort）。【重构注 2026-09-12】真机验收后重构：① 测试预览由排序测试改为正则匹配测试（输入文本 + 「测试」按钮手动触发、删 useCallback+useEffect 实时联动，直调 core matchSmartSortPattern，结果区 monospace 逐匹配显示文本+捕获组，输入变化清结果）；② 正则撤全屏编辑（删 PatternFullscreenEditorScreen 与 PatternEditor 路由），改自适应高度 multiline 输入（内容增高、约 7 行封顶内部滚动）；③ example 字段更名 description（表单标签「描述」、列表小字同步）；④ 编辑屏键盘避让由 ScreenFormLayout 内建（ScrollView+keyboardShouldPersistTaps+裁切收缩）覆盖。【重构注 2026-09-12 二次】测试结果区改高亮渲染：按 matches.index 偏移把原文切成普通段/匹配段（匹配段 tokens.primary 变色，嵌套 Text 继承 monospace）+ 底部「共 N 处匹配」计数（0 处显「无匹配」）；描述字段改自适应多行（AutoGrowMultilineInput 与正则同款 auto-grow，封顶 120）。【D13 注 2026-09-13】编辑器增「捕获数字」下拉（PickerListModal 值行模式照 DirectoryRuleSheet，三选项：智能数字/固定最大/固定最小，档位描述 hint 随动）；测试预览每处匹配下显示提取元组小字（tuple 文案，null 显「无序号」，固定档显哨兵文案）；matchSmartSortPattern 调用带 captureKind。
- Step 14 — phase-mobile-ui — blocking: yes — qa: auto：`DirectoryRuleSheet.tsx` 选项 + 文案（`:35`/`:131`）；`vfs-direct-children-order.ts` `orderedDirectChildPaths` 经 runtime 取 `listCompiledRules` 传入可选参；扩展 `__tests__/vfs-direct-children-order.test.ts`；`npm run typecheck`（官方脚本，记忆 #38）。【重构注 2026-09-12】排序方式由 4-chip 横排换行改为 PickerListModal 单选选择器（行内显当前值 + 点击弹底部列表，选项序智能排序/文件名称/创建/更新，照 ModelPickerModal 先例）：换行 chip 把面板顶出滚动区，改后表单收进一屏免纵向滚；排序方向（2 chip）与其余文件填充（3 chip）均单行不换行，维持 chip 原样。
...
（按 sort_order 优先级排列，首命中负责提取；flags 缺省为空字符串，seed 携带固定 sort_order 与 flags，与上表一致；已 node 验证七条均可编译、对样例命中且提取正确。）

【D13 注 2026-09-13】内置 4→7 条：三条 fixed 档排最前（序章类 1 / 终章类 2 / 番外类 3），四条数字规则显式 `captureKind='smart'` 顺延 4..7（行为零变化）；存量库重跑 seed 时新行 sortOrder 1..3 与存量四行旧 sort_order 撞号，listOrdered 以 (sort_order, rule_id) 全序决胜，固定档特殊词与数字规则无抢占冲突，调序/恢复默认会整表重编号收敛。

【重构注 2026-09-12】example 字段已更名 description，内置规则值为描述文字（不再是可命中的示例文件名）；下列各行 example 仅作 pattern 行为验证的历史记录保留（常量锁测试改用内联样例文本）。

```text
builtin-zh-prologue       「序章/开篇」                      sort_order=1  flags=''  captureKind=fixed_min
pattern: ^(序章?|楔子|引子|前言|开篇)
描述：序章/楔子/引子等开篇文字，排在所有章节之前（命中即哨兵元组 (-∞,)）
sample: 楔子 起源 → (-∞,)

builtin-zh-finale         「终章/收尾」                      sort_order=2  flags=''  captureKind=fixed_max
pattern: ^(终章|尾声|后记|完结|finale)
描述：终章/尾声/后记等收尾文字，排在所有章节之后（命中即哨兵元组 (+∞,)）
sample: 终章 大结局 → (+∞,)

builtin-zh-extra          「番外/外传」                      sort_order=3  flags=''  captureKind=fixed_max
pattern: ^(番外|外传|if篇?)
描述：番外/外传等衍生内容，排在终章之后（同为 fixed_max 同值，文件名决胜：
「番」U+756A <「终」U+7EC8 → 番外在前；D13 定稿同值 tiebreak，不加权重）
sample: 番外一 日常 → (+∞,)

builtin-zh-volume-chapter 「中文卷章复合」                    sort_order=4  flags=''  captureKind=smart
pattern: 第[ \t]{0,2}(NUM)[ \t]{0,2}卷[ \t]*[-—·.、]?[ \t]*第[ \t]{0,2}(NUM)[ \t]{0,2}章
（NUM = [0-9〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,12}，两捕获组：卷、章）
example: 第2卷 第13章 → [2,13]

builtin-zh-chapter        「中文序号章节」                    sort_order=5  flags=''  captureKind=smart
pattern: 第[ \t]{0,2}([0-9〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,12})[ \t]{0,2}(?:章|节|集|部|篇|回|卷)
example: 第十二章 风起 → [12]

builtin-en-chapter        「英文章节」                        sort_order=6  flags='i'  captureKind=smart
pattern: (?:chapter|section|part|episode)[ \t]*[.．]?[ \t]*([0-9]{1,6})
example: Chapter 12 → [12]（大小写不敏感依赖 flags 'i'）

builtin-numeric           「数字序号开头」                    sort_order=7  flags=''  captureKind=smart
pattern: ^[ \t]*([0-9]{1,6})[ \t]*(?:[、.．\-—_]|$)
example: 001、开端 → [1]
```
