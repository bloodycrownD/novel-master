...
- T-A6 — blocking: yes —（第三轮新增）duckduckgo 适配器：mock HTML 响应断言请求（GET+q+UA）、解析（标题/uddg 解码/snippet/实体解码/广告块跳过）、domainFilter 客户端过滤、0 可解析结果抛 invalid、恒 configured、链尾兜底（无任何 key/baseUrl 时链=[duckduckgo] 且请求真实发出——T-S1 语义随之更新）。
