...
2. **配置存储（core 纯逻辑 + 双端薄壳）**：SKSP ref `search/{engineId}/apiKey` + KKV 模块 `nm-search`（`defaultEngine`、`searxngBaseUrl`），形态照 cloud-sync 先例（`cloud-sync/s3-secret-key` + `nm-cloud-sync`）。
...
| `service/agent/logic/run-agent-turn.ts` | `AgentTurnRuntimePort` 加 `searchConfig?: SearchConfigStore`（可选声明——旧测试 mock 零改动；装配见 Step 3：desktop/mobile runtime 各补一行、CLI 无 kkv 恒未配置）；主/子两装配点（L505/L739）注入 `search` 闭包，vfs/workplace/sessionKkv 复用装配点既有实例（子代理天然落父工作区） |
...
| `shared/ipc-types.ts` | `nm:search/getConfig`、`saveEngineKey`、`clearEngineKey`、`setSearxngBaseUrl`、`setDefaultEngine`（DTO 不含 key 明文，只有 `configured: boolean`） |
...
| `renderer/features/settings/SearchEnginesView.tsx`（新） | `SettingsFormSection` + 每引擎一张 `SettingsSection` 卡（引擎名 + `ApiKeyStatusTag` + `SettingsField`（bocha/tavily/brave 填 key，searxng 填 baseUrl）+ 清除按钮）+ 默认引擎选择（`SegmentedControl`，仅列 `configured=true` 引擎——searxng 以 baseUrl 非空计，与解析链同口径）+ 保存 footer |
| `renderer/features/settings/settings-nav.ts` + `layout/SettingsOverlay.tsx` | AI 组加 `searchEngines` 项（ViewId union / NAV / TOP_LEVEL / renderContent case） |
...
| `src/screens/stack/SearchEnginesScreen.tsx`（新） | `ScreenFormLayout` + 每引擎 `FormSectionCard`（`FormTextInput` key/baseUrl + `ApiKeyStatusTag`）+ `FormChipGroup` 默认引擎（仅列 `configured=true` 引擎，与解析链同口径）+ `StickyFormFooter` |
| `src/navigation/types.ts` + `RootNavigator.tsx` | `SearchEngines` 路由（withStackLayout） |
| `src/screens/tabs/ProfileTabScreen.tsx` | CONFIG_MENU 加「AI 搜索」 |
...
- Step 8 — phase-desktop-ui — blocking: yes — qa: manual_user：IPC 四通道 + handler + invoke 封装 + `SearchEnginesView` + 导航接入 + 双处 hint 文案；表单校验仅非空/URL 形状（无连通性测试，PRD 口径）。
- Step 9 — phase-mobile-ui — blocking: yes — qa: manual_user：`search-config.store` + runtime 装配 + `SearchEnginesScreen` + 导航 + CONFIG_MENU + hint 文案；mobile 测试跑法 `NODE_ENV=test npx jest`（约束 #22）、类型检查用官方 `typecheck` 脚本（约束 #38）。
...
- T-C1 — blocking: yes — search-config 读写（SKSP/KKV mock）：保存后 `readSearchConfig` 的 configured 状态、清除后回落、searxng 仅配 baseUrl 即 configured=true、明文不出现在任何返回值。
- T-C2 — blocking: yes — key 无效（401）时错误信息不含 key 明文。
- T-S1 — blocking: yes — 未配置任何引擎：run 返回含配置入口指引的提示，不抛错。
- T-S2 — blocking: yes — 默认引擎解析链（input.engine > defaultEngine 偏好 > 第一个 configured）；含 searxng-only 用例：仅配 baseUrl（无任何 key）也能被解析链命中。
...
- T-D1 / T-M1 — blocking: no — qa: manual_user — 桌面/移动配置页：填 key 保存后 configured 状态刷新、searxng baseUrl 校验、默认引擎切换、清除 key；走查项含「落盘文件在双端文件树可见」；合并后用户双端走查（mobile 无 webview 改动，metro reload 即生效）。
