...
- **openai 出站补齐（2026-08-27 追加）**：原设计 Step 3「openai 协议行为不因开关改变（mapper 全量过滤 thinking）」废止——openai 出站改为将 assistant 消息的 thinking 块文本拼接为 `reasoning_content` 字段回传（openai 兼容生态通用约定，DeepSeek 官方要求工具轮回传 CoT；对齐 opencode / deepseek-harness 实践），redacted_thinking（anthropic 加密负载）无对应字段、丢弃。上游容量 1 剥离先于 mapper，回传集合即容量 1 保留集合；关态 / 档位 off 时 thinking 已剥光，自然无 `reasoning_content` 字段。纯 thinking 的 assistant 消息出站不丢：`content` 置 null + `reasoning_content` 回传。存量断言「omits thinking blocks」按新行为更新为断言回传。
