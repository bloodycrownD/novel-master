...
开态保留策略从「最新一轮（最后一条真实用户输入边界）」改为「**容量 1**」，其余章节描述保留原案、以本节为准：

- **变更内容**：开 = 仅保留「最后一条含 thinking / redacted_thinking 块的 assistant 消息」的全部思考块（整条原样含签名，混合块一起保留），其余 assistant 全剥。关态协议最低保留与档位前置门不变。
- **动机**：按用户输入边界划「最新一轮」在嵌套工具循环场景会出现多条 thinking 并存（上一回合全部 + 本回合全部）；容量 1 把思考当作容量为 1 的消息槽，跨轮自然可见（上一轮思考对新一轮可见）、新一轮思考生成后自动刷新，任何时刻上下文内至多一条 thinking。业界对照（opencode / deepseek-harness 均全量回传）容量 1 为中间档。
- **协议兼容**：容量 1 在活跃工具循环中命中的消息与关态协议最低保留是同一条，自动满足协议下限；关态「最后一条含 tool_use 的 assistant」判据在开态放宽为「最后一条含思考块的 assistant」（覆盖纯文本回复的跨轮可见）。
- **实现影响**：`findRealUserBoundaryIndex` / `isRealUserInput` / `"prompt:"` 前缀排除规则删除（容量 1 只看 assistant，合成 user 消息天然不影响）；T-TC1/2/5/6/7、T-PV2 断言按容量 1 重写；agent-runner 插入顺序保持不变（顺序理由中的边界误判已不存在，注释同步）；双端开关描述改为「开启后，最近一次的思考内容进入提示词，更早的不进入。」
- **openai 出站补齐（2026-08-27 追加）**：原设计 Step 3「openai 协议行为不因开关改变（mapper 全量过滤 thinking）」废止——openai 出站改为将 assistant 消息的 thinking 块文本拼接为 `reasoning_content` 字段回传（openai 兼容生态通用约定，对齐 opencode / deepseek-harness 实践），redacted_thinking（anthropic 加密负载）无对应字段、丢弃。纯 thinking 的 assistant 消息出站不丢：`content` 置 null + `reasoning_content` 回传。存量断言「omits thinking blocks」按新行为更新为断言回传。
- **撤销容量 1、开态改为全量保留（2026-08-27 第二次修订，用户拍板）**：容量 1（仅保留最后一条含思考块的 assistant）实测后撤销——标准方案（全量回传）是 opencode / deepseek-harness 双双验证过的主流路径，端点兼容性有背书；自创的部分回传语义存在未知兼容风险。开态 = 全量保留（消息原样返回，含全部 thinking / redacted 与签名），关态协议最低保留与档位前置门不变；openai 出站全量回传 `reasoning_content`。双端开关描述改为「开启后，模型的思考内容进入后续提示词，关闭则不进入。」已知限制：GLM 智谱官方端点对输入 `reasoning_content` 字段可能忽略（实测部分回传时不可见，全量回传待验证），如后续确认忽略，文本标签方案（<thinking> 拼接 assistant content）作为备选记录于此。
- **默认值改为关闭（2026-08-27 第三次修订，用户拍板）**：偏好默认 true → false（正文「默认开」废止），agent-runner 兜底链（未注入 preferences / KKV 坏值回退）同步 false——默认行为与旧版「思考不进上下文」一致，openai 出站不发 `reasoning_content`，零兼容风险；需要跨轮思考记忆的用户手动开启。T-PF1 与 MF-3 断言按默认关更新。
