...
| 聊天页 `#chatPage` 左侧抽屉（`#chatDrawer`，**非**项目/会话操作抽屉） | 进入会话聊天后，顶栏 ☰ 打开 **聊天抽屉**：展示当前模型 + **切换模型**；选中写入 **工作区当前模型**（与「我的」同一字段） |
| 会话操作 | 聊天顶栏 **会话操作** 按钮 → `#sessionActionsDrawer`（真实提示词、会话日志）；**不含**模型切换 |
