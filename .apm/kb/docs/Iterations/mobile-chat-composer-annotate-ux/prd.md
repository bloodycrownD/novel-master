...
6. **消息正文批注（新）**：与工作区文件批注区分——**不**产生状态 chip、**不**在气泡上画下划线；在 Composer 中以类似文件引用的 **tag** 表达；删除 tag 即删除对应批注；发送后 **须进入模型上下文**。
...
| 消息正文批注可用 | 可对消息正文划词添加批注（须解析到 messageId）→ Composer 出现 tag（无 chip、气泡无下划线）→ 删 tag 即删批注 → 发送后模型能看到该批注含义；`userContent` 已剥离消息批注 span，原文含 `@path` 时亦不误扫 attach |
...
| 需要摘抄消息片段的人 | 用右上角菜单操作消息，同时能在气泡上划词复制；需要时对某段助手/用户正文做批注并以 tag 发给模型 |
...
6. **消息正文批注 → Composer tag（新能力）**  
   - 入口：在消息气泡正文上划词添加批注（transcript 选区菜单对齐文件预览：须含「批注」+「复制」；由选区上溯消息 row 的 `messageId` + 选区文本 → 宿主录入；「复制」走系统剪贴板）。解析不到 `messageId` 则取消录入。  
...
6. **消息正文批注**：划词（选区上溯 `messageId` → 宿主录入；划词菜单含「批注」+「复制」）→ Composer tag（无 chip、气泡无下划线；发送前剥离消息批注 span 得 `userContent`）→ 删 tag 即删批注 → 发送后进模型并清空对应 tag。
...
- [ ] **Given** 助手或用户消息正文，**When** 划词，**Then** 选区菜单含「批注」与「复制」；点「复制」可将选中片段写入系统剪贴板（自定义 menu 盖掉原生 Copy 后仍可复制；不被长按主菜单抢光）。  
- [ ] **Given** 菜单「复制」，**When** 触发，**Then** 仍复制整条可编辑正文（与划词复制并存）。  

### 消息正文批注

- [ ] **Given** 用户在消息气泡正文划词并添加批注，**When** 查看 Composer，**Then** 出现对应 tag，且 **不**出现 `批注:` 状态 chip，气泡上 **无** 下划线。  
- [ ] **Given** 划词时无法解析所属消息 `messageId`（或空选区），**When** 触发批注入口，**Then** 取消录入，不写草稿、不插 tag。  
...
| 划词 messageId | 须从选区上溯消息 row | SPEC 已钉 `getSelection` → `.row.message[data-id]`；失败取消录入 |
