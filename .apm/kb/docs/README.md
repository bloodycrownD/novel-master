...
| token-counting | [Iterations/token-counting/](Iterations/token-counting/) | tiktoken / heuristic token 计数 |
| sksp | [Iterations/sksp/](Iterations/sksp/) | SKSP Android + CLI provider |

Monorepo 布局见 [monorepo.md](monorepo.md)。会话开始：`apm read`（读 `.apm/memory/persist.md` + `dynamic.md`）。
